...
class Discriminator(nn.Sequential):
    def __init__(
        ...
        conv_chans: List[int],
        k_size: int,
        init_params: Optional[Callable[[nn.Module, str], Any]] = None,
        dropout: float = 0.,
        bn: bool = True
    ) -> None:
        ...
        for in_chan, out_chan in zip(conv_chans[:-1], conv_chans[1:]):
            _layers.append(nn.Conv2d(in_chan, out_chan, k_size, stride=2, padding=k_size // 2, bias=not bn))
            if bn:
                _layers.append(nn.BatchNorm2d(out_chan))
            _layers.append(nn.LeakyReLU(0.2))
            if dropout > 0:
                _layers.append(nn.Dropout(dropout))

        # FC layers
        _layers.append(nn.Flatten(1))
        _layers.append(nn.Linear(int(conv_chans[-1] * (img_size / 2 ** (len(conv_chans) - 1)) ** 2), 1))
...
class Generator(nn.Sequential):
    def __init__(
        ...
        _layers.append(nn.Linear(latent_size, conv_chans[0] * _reduced_size ** 2))
        _layers.append(UnFlatten((conv_chans[0], _reduced_size, _reduced_size)))

        # Conv layers
        _idx = 1
        for in_chan, out_chan in zip(conv_chans[:-1], conv_chans[1:]):
            _layers.append(nn.ConvTranspose2d(in_chan, out_chan, k_size, stride=2, padding=k_size // 2,
        ...
        self.apply(init_params)
        last_conv_idx = len(self) - 2
        getattr(self, str(last_conv_idx)).apply(partial(init_params, nonlinearity='tanh'))
